...
- 每篇笔记强制按 STAR 展开：Situation 交代场景与失败模式，Task 定义输入输出、目标和约束；Action 写成连续的中文知识博客，先用一个具体案例串起全流程，再按论文 Method 原结构覆盖每个子模块、真实输入、逐步操作、输出接口、公式与训练/推理差异；Result 用指标、基线、消融与证据收束。禁止“翻译/解释”模板、通用占位句和省略子模块。
