...
一个可配置、可审计、跨 harness、跨平台的每日论文推送 Skill。它负责近期论文搜索、历史去重、按类别配额选文、逐论文原图归档、详细中文笔记契约、Zotero PDF 深链与分类，以及失败后可重试的每日定时运行。
...
- “核心创新”强制拆成至少三个可复核阶段，逐阶段说明输入、操作、输出、目的和论文证据；Top 推荐要求更长、更细。
