...
- Situation: research/application setting, concrete failure mode, consequence, and why prior approaches are insufficient. Present the understanding directly; never narrate source lookup or translation.
- Task: exact input/output, objective, constraints/assumptions, evaluation target, and scope boundary.
- Action: write a cohesive Chinese knowledge blog, not alternating translation/explanation blocks. First recover the paper's own Method outline, give a concrete end-to-end workflow, and introduce one running example. If Method has N explicit first-level subsections or named components, write exactly N `### 方法部分 N：中文标题（Original Method Heading）` sections in the same order; a one-part Method gets one section. Recover every nested subsection and distinct named operation as a natural `####` subheading. For each, name its real inputs, ordered operations and branches, outputs and downstream consumer, purpose, failure prevented, formula/selection rule, training or runtime timing, and the running example's intermediate state. Store source provenance only in JSON evidence fields. Never use `翻译：`, `解释：`, source-verification or extraction commentary, raw English excerpts, generic upstream/downstream filler, a detached formula template, cross-domain catch-all examples, or omitted submodules. Add one corresponding `method_stages` object per first-level part with `overview`, `walkthrough`, structured `submodules`, and equation evidence as specified by the contract.
